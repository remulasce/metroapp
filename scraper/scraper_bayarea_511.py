...
if len(agencyList) == 0:
    ...
    agencyListRequestString = "https://api.511.org/transit/operators?api_key=f036cd72-4465-425d-9ce2-df2478c7f804"
...
for agencyParam in agencyList:
    ...
    searchString = "http://services.my511.org/Transit2.0/GetRoutesForAgency.aspx?token=7a0b4b7b-6a70-46d7-85aa-8a202fc44471&agencyName=" + agencyName.replace(
        ' ', '%20')
                ...
                for child in routeObject.find("RouteDirectionList").find("RouteDirection").find("StopList"):
                    if child.tag == "Stop":
                        ...
                        if not str(stoptag) in stops_dict:
                            print(
                                "Skipping " + stoptag + ", " + stopname + " on " + routeTag + "~" + routeDir + " for " + agencytitle + " because we couldn't find the stop in stops.txt")
                            continue
