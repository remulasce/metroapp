 ...
 *
 * <p>Asynctask searches SQL for routes to a stop, then fills in the colorbars for a listitem.
 *
 * <p>Made for the autocomplete dropdown box. Supports multiple colors per stop, so make sure the
 * viewgroup its handed can expand safely.
 */
public class RouteColorerTask extends AsyncTask<Collection<Stop>, Void, Collection<RouteColor>> {
  ...
  public RouteColorerTask(
      ViewGroup colorBarLayout, StopRoutesTranslator stopRoutes, RouteColorer routeColorer) {
  ...
  private void fillinColorBars(ViewGroup colorsLayout, Collection<RouteColor> colorBars) {
    LayoutInflater inflater =
        (LayoutInflater)
            colorsLayout.getContext().getSystemService(Context.LAYOUT_INFLATER_SERVICE);
    ...
    for (RouteColor c : colorBars) {
      ...
      // If there's no recycled views left, make one.
        ...
        sizeChanged = true;
        updateColorView =
            inflater.inflate(R.layout.omnibar_dropdown_color_bar, colorsLayout, false);
