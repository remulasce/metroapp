 ...
 *
 * <p>Just routes everything through the preloaded sql provider.
 */
public class MetroStaticsProvider
    implements StopLocationTranslator, StopNameTranslator, AutoCompleteCombinedFiller {
  private static final String TAG = "MetroStaticsProvider";
  private final InstalledAgencyChecker installedAgencyChecker;

  private Map<Agency, SQLPreloadedStopsReader> regionalStopsReaders =
      new HashMap<Agency, SQLPreloadedStopsReader>();
  ...
  private void setupRegion(Context context) {
    Collection<Agency> installedAgencies =
        RegionalizationHelper.getInstance().getInstalledAgencies();
    if (installedAgencies != null) {
      for (Agency agency : installedAgencies) {
        if (!regionalStopsReaders.containsKey(agency)) {
          // add agency
          SQLPreloadedStopsReader reader =
              new SQLPreloadedStopsReader(
                  context, installedAgencyChecker.getDatabaseFileNameForAgency(agency), agency);
  ...
  public Collection<Route> getRoutesToStop(Stop stop) {
    ...
    if (reader == null) {
      Log.w(
          TAG, "Don't have preloaded region " + stopAgency + ", can't get routes to stop " + stop);
      return ret;
    }
  ...
  public Collection<OmniAutoCompleteEntry> autocompleteLocationSuggestions(
    ...
    for (Agency a : regionalStopsReaders.keySet()) {
      ...
      Collection<OmniAutoCompleteEntry> entries =
          reader.autocompleteLocationSuggestions(locations, maxDistanceMeters, maxResults);
