...
/**
 * Old stops.txt sql handler Removed in order to use preloded SQL tables
 *
 * <p>Though, actually this is what created the prefilled tables we now ship.
 */
public class GTFSStopsReader extends SQLiteOpenHelper
  ...
  public BasicLocation getStopLocation(Stop stop) {
    ...
    Collection<SQLEntry> entries =
        getMatchingEntriesRaw(makeStopLocationRequest(stop.getStopID()), getReadableDatabase());
  ...
  public Collection<OmniAutoCompleteEntry> autocompleteStopName(String input) {
    ...
    for (String s : split) {
      ...
      //            Collection<SQLEntry> matchingEntries =
      // getMatchingEntriesRaw(makeAutoCompleteNameRequest(s), db);
  ...
  private Collection<SQLEntry> getMatchingEntries(
    ...
    try {
      ...
      while ((line = file.readLine()) != null) {
        ...
        if (entries % 1000 == 0) {
          Log.d(
              TAG,
              "Still updating database; "
                  + entries
                  + " entries so far in "
                  + Tracking.timeSpent(start)
                  + "ms");
        }
      }
