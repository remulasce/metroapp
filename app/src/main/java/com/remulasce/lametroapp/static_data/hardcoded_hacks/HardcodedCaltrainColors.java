...
/**
 * Hardcoded Caltrain rail colors Uses colors from caltrain schedule for local, limited, bullet
 * service
 */
public class HardcodedCaltrainColors implements RouteColorer {
  ...
  public RouteColor getColor(Route r) {
    ...
    if (r.getAgency().matches("caltrain")) {
      if (raw.equals("LOCAL")) {
        return new RouteColor("#77787B");
      }
      if (raw.equals("LIMITED")) {
        return new RouteColor("#FEF0B5");
      }
      if (raw.equals("BABY BULLET")) {
        return new RouteColor("#E31837");
      }
    }
