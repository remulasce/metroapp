 ...
 *
 * <p>UI layer turns Trips into Views for Android
 */
public class AndroidMultiArrivalDisplay implements AndroidDisplay {
  ...
  public View getView(ViewGroup parent, Context context, View recycleView) {
    ...
    for (Arrival a : trip.parentArrival.getArrivals()) {
      ...
      // If there's no recycled views left, make one.
        ...
        updateTimeView =
            (RelativeLayout)
                inflater.inflate(R.layout.trip_arrival_vehicle_row, timesLayout, false);
        updateTimeView.setTag(a);

        timesLayout.addView(updateTimeView);
      }

      TextView prediction_text_minutes =
          (TextView) updateTimeView.findViewById(R.id.prediction_time_minutes);
      TextView prediction_text_seconds =
          (TextView) updateTimeView.findViewById(R.id.prediction_time_seconds);
    ...
    for (Arrival a : trip.parentArrival.getArrivals()) {
      if (a.isInScope() && a.getEstimatedArrivalSeconds() > 0) {
        ...
        if (a.getVehicleNum() != null) {
          button.setText(
              "Vehicle "
                  + a.getVehicleNum().getString()
                  + " "
                  + LaMetroUtil.timeToDisplay((int) a.getEstimatedArrivalSeconds()));

          button.setTag(a);
          radios.addView(button);
        } else {
          // We need vehicle numbers to distinguish between different arrivals. If we have no veh
          // numbers, we can only track "first arrival".
  ...
  private void setTrackingEventListeners(NumberPicker time, RadioGroup vehicleRadio) {
    ...
    vehicleRadio.setOnCheckedChangeListener(
  ...
  private void launchNotificationConfirmation(
      final Context context, final View dialogView, Boolean allowOk) {
    final NumberPicker time = dialogView.findViewById(R.id.notify_dialog_time);
    final RadioGroup vehicleRadio =
        (RadioGroup) dialogView.findViewById(R.id.trip_options_radio_group);
            ...
            .setPositiveButton(
                ...
                new DialogInterface.OnClickListener() {
                  ...
                  public void onClick(DialogInterface dialog, int which) {
                    ...
                    NotifyServiceManager.SetNotifyService(
                        trip.parentArrival.getStop(),
                        trip.parentArrival.getRoute(),
                        trip.parentArrival.getDirection(),
                        vehicle,
                        seconds,
                        context);
                  }
