 ...
 *
 * <p>It mainly just checks how long it's been since each Prediction has been updated, against how
 * frequently the Prediction wants to be updated.
 *
 * <p>Predictions helpfully just have a getRequestString and handleResponse method. So we don't have
 * to do any parsing here, just feed Predictions with an update every so often.
 *
 * <p>It doesn't do any fancy consolidation or staggering. It just has a minimum interval to prevent
 * ridiculously high requested frequencies, and it shuts down and restarts everything when the app
 * opens/closes.
 */
public class PredictionManager {
  ...
  /**
   * Start getting updates for this Prediction. Starts the entire PredictionManager if it is not yet
   * started. An update will be forced in startUpdatingIfNotStarted, bypassing the normal wait
   * period.
   *
  ...
  /**
   * Stops tracking a particular prediction Doesn't shut down the service when empty.
   *
  ...
  /**
   * Globally stops tracking on all predictions This is for when the app is closed. Resume all
   * tracking with resumeTracking The predictions are maintained while paused, just a new updater
  ...
  /**
   * Globally resume tracking on all predictions Creates a new thread to do that, if necessary.
   *
   * <p>Also immediately unlocks the wait object to begin updating immediately.
   */
  ...
  /**
   * The update thread wait()s on the updateObject for xxx ms. When the object is notify()ed, the
   * wait immediately ends This lets you bypass the waiting period.
   */
  ...
  /**
   * This just compares requestedUpdateInterval vs time since last update on all the predictions. It
   * starts threads to do each actual net update.
   */
