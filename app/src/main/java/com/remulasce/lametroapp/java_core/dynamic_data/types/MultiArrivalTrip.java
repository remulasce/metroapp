...
public class MultiArrivalTrip extends Trip {
  ...
  public boolean equals(Object obj) {
    if (!(obj instanceof MultiArrivalTrip)) return false;
  ...
  public double getCurrentDistanceToStop() {
    ...
    if (retriever != null) {
      ...
      Log.d(TAG, String.format("Location retriever returned: %f", currentDistanceToStop));
  ...
  public float getPriority() {
    ...
    // ~20 miles
    proximity += Math.max(0, .2f * (float) (1 - (distance / 32000)));
    // ~2 miles
    proximity += Math.max(0, .8f * (float) (1 - (distance / 3200)));
